...
const controller = createController(state, (events) => {
  const scene = game.registry.get('scene');
  if (scene) {
    // Changing floors (down OR up) swaps in a different map, so rebuild the
    // tile/entity visuals; otherwise animate the turn (glide movers, play
    // combat effects — applyTurn plays the events itself).
